...
에디터 버전(블록 기반 자유 편집)을 다시 보고 싶을 때:

```bash
# 에디터 버전이 살아있던 마지막 커밋 (리팩토링 직전)
git stash           # 현재 작업 임시 저장
git checkout d862ce4  # 에디터 버전 마지막 커밋
npm run dev         # http://localhost:3000 에서 확인

# 돌아오기
git checkout v2     # 현재 버전으로 복귀
git stash pop       # 임시 저장 복원
```

- `d862ce4` = 에디터+정형화 공존하던 마지막 커밋
- `82daed9` = 리포트 생성 화면(에디터) 최초 구축
- `0f37c0b` = v2 위젯 블록 시스템 최초 구축
