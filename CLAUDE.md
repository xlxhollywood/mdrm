...
src/
  ...
  components/
    ...
    AppHeader.jsx                        상단 헤더
    RightPanel.jsx                       우측 옵션 패널 (위젯 옵션 / 문서 설정) - 진입점
    canvas/
      GridCanvas.jsx                     Grid 모드 캔버스 + PlacedCard + 드래그 재정렬
      WordCanvas.jsx                     Word 모드 캔버스 오케스트레이터
    ...
    right-panel/                         RightPanel 서브 컴포넌트들
      shared.jsx                         공용 UI 요소
      SystemSelect.jsx                   시스템 선택 트리
      TablePanel.jsx                     표 옵션 패널
      WordDocPanel.jsx                   Word 문서 설정 패널 (용지·방향·여백·발행)
    ...
    widgets/
      ...
      PreviewFlowchart.jsx               플로우차트 미리보기
      PreviewHistory.jsx                 변경이력 미리보기
      PreviewHistorySimple.jsx           변경이력 심플 미리보기
      PreviewHistoryTable.jsx            변경이력 테이블 미리보기
      PreviewTable.jsx                   테이블 미리보기
```
...
## 기능 구현 후 자체 테스트 체크리스트

기능 구현이 끝나면 반드시 아래 항목을 직접 확인할 것.

- [ ] 새 블록/요소 생성 후 커서(포커스)가 올바른 위치에 있는가
- [ ] 새 블록/요소에서 텍스트 입력이 되는가
- [ ] `/` 슬래시 메뉴가 트리거되는가
...
- **자동 커밋**: 사용자가 전혀 다른 새 작업을 요청하면 이전 작업이 완료된 것으로 간주하고, 별도 요청 없이 자동으로 커밋을 남길 것
